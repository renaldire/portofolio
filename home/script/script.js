var btnApps = document.getElementsByClassName('apps');
btnApps[0].style.backgroundColor="#0D47A1";
...
{
	$('html,body').animate({ scrollTop: $("#pnlSkill").offset().top-60 }, 800);
...
{
	$('html,body').animate({ scrollTop: $("#pnlContact").offset().top }, 800);
	$('.navBar-content').css('border-top','3px solid rgba(100,100,100,0.75)');
}

function scrolltoApps()
{
	$('html,body').animate({ scrollTop: $("#mainPnlApp").offset().top-60 }, 800);
...
function showAppDetail(id)
{
	...
	$('#mainPnlApp').animate({'margin-left':'-='+$page+'vw'},1000);
	currentPage=id;

	for(i=0;i<btnApps.length;i++)
	{
		btnApps[i].style.backgroundColor="#2196F3";
	}
	btnApps[id-1].style.backgroundColor="#0D47A1";
}
...
$('#lblIntro').animate({ "margin-top": "+=45vh" },700,function(){
	setTimeout(function(){
		$('#lblIntro').fadeOut(function(){
			...
			$('#lblIntro').fadeIn(1000,function(){
				...
				$('#introBg').click(function(){
					$('#introBg').animate({ "margin-top": "-=100vh" },1000,function(){
						$('#introBg').hide();
						$("#lblName").fadeIn(3000);
